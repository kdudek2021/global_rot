...
d1 = np.loadtxt("global_rot_extent_outer_masses_ratio_5_height_30cm.dat")    # wczytuje plik (komentarze sa pominiete)
#d2 = np.loadtxt("time_beeman_hexachiral_l=2_0_a_3_elements_theta0_1_theta_eq_270_dt=1e-5.dat")
d2 = np.loadtxt("global_rot_extent_outer_masses_ratio_5_h=35cm.dat")
d3 = np.loadtxt("global_rot_extent_outer_masses_ratio_5_h=40cm.dat")
...
plt.plot(x1, y1, linestyle = '-', linewidth=w1, color='black', label = r'$h = 30$')

#line2, = plt.plot(x2, y2, linestyle = '-', linewidth=w1, color='blue', label = r'$l_{a}/l_{b} = 5$')
#line2.set_dashes([14,5,3,5])

line3, = plt.plot(x2, y2, linestyle = '-', linewidth=w1, color='green', label = r'$h = 35$')
line3.set_dashes([10, 5, 3, 5, 3, 5])

line4, = plt.plot(x3, y3, linestyle = '--', linewidth=w1, color='purple', label = r'$h = 40$')
...
# Jak chcesz zapisac:
plt.savefig("global_rotation_different_initial_height.svg")

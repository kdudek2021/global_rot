...
class YZ_Visualisation_mag_moments_entire_structure():
    ...
    def drawing(self, all_pos_lists, two_body_bonds_indices):#, mag_vectors_to_visualise, index_of_mag_mom_to_visualise):
        ...
        points_top_y = points_coordinates[0]
        points_top_z = points_coordinates[1]
        points_bottom_y = points_coordinates[2]
        points_bottom_z = points_coordinates[3]
        ...
        for j in range(len(two_body_bonds_indices)):
            ...
            line_segment = [ (np.array([self.points[point1_index][1], self.points[point1_index][2]]) ).tolist(), (np.array([self.points[point2_index][1], self.points[point2_index][2]]) ).tolist()]
        ...
        line_initial_segment = [(x).tolist(), ().tolist() ]
        ...
        plt.axis([-0.15, 0.15, -0.15, 0.5])
        
        ax = plt.gca()		#I move both xticks abd yticks away from the graph
        
        ax.plot(points_top_y, points_top_z, 'bo', markersize = 12)
        ax.plot(points_bottom_y, points_bottom_z, 'ro', markersize = 12)
        ...
        #plt.show()
        plt.savefig("structure_YZ_at_t = "+str(self.i_time)+".png") 
    ...
    def additional_rot_lines(self):
        ...
        for i in range(2):
            ...
            if i == 0:
                ...
                point1_middle_extended = np.array([0.0, 0.0, point2_middle[2]])#point1_middle - vector_initial * 1.0
                ...
                point1_middle_extended = np.array([0.0, 0.0, point2_middle[2]])#point1_middle - vector_current * 1.0
                point2_middle_extended = np.array([0.0, 0.0, point2_middle[2]]) + (vector_current / vector_current_mag) * 0.35#point1_middle + vector_current * 2.0
